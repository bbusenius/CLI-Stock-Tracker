...
This script orchestrates the workflow by loading the list of tickers, fetching financial data
for each ticker using the Finnhub API, and displaying the results in a formatted table.
It serves as the entry point for the command-line interface, integrating the configuration,
API client, and display modules to provide a seamless user experience.
"""
...
def main():
    ...
    1. Loads the list of tickers from 'tickers.json'.
    2. Initializes the Finnhub API client using the API key from the environment variable.
    3. Fetches financial data for each ticker.
    4. Displays the collected data in a formatted table using the rich library.
    ...
    Notes:
        ...
        - The function fetches data sequentially for each ticker, respecting the Finnhub API's
          rate limit of 60 calls per minute (up to 12 tickers without delays, as each ticker
          requires up to 5 calls).
    ...
    for ticker in tickers:
        ticker_data = fetch_ticker_data(client, ticker)
